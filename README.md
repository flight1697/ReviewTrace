...
点击 **选择文件并生成报告** 可以选择 `.json` 或 `.csv` 文件。导入的数据会走同一个 workflow API，并返回原始评论、清洗结果、分类结果、发现、需求、版本计划、产品需求文档草案、QA 用例和追溯校验。
...
| 阶段 | 实现方式 | 选择理由 |
| --- | --- | --- |
| 范围 | 用户目标驱动的评论子集选择 + 模型输出的 `scope` + 后端范围归一化 | 用户目标可能包含订阅、低评分、版本或可用性限制，需要先收敛参与语义分析的评论范围，并把范围评论 ID 展示出来。 |
| 评论采集 | 确定性 HTTP 请求 | 数据源和请求频率要可重复、可解释，避免对 Apple RSS 形成重试风暴。 |
| 清洗去重 | 确定性规则 | 空评论剔除、标题正文指纹去重、评分统计都应稳定可复现。 |
| 语义分析 | DeepSeek/OpenAI 模型驱动，缺 key 时不生成发现 | 主题发现和问题整合需要泛化到未见应用、混合语言和新目标，不能只靠固定关键词。 |
...
后端会返回 `analysisScope` 和 `stageReports`。`analysisScope` 还会包含 `selectionSummary`、`filteringRules` 和 `excludedReviewIds`，用于说明为什么当前评论被纳入或排除。前端会展示分析范围、范围评论 ID、过滤说明、证据信号、约束、不确定性，以及每个阶段的中间结果、修订记录和校验结果。

前端默认调用流式接口 `POST /workflow/runs/stream`。后端以 NDJSON 逐行返回：

- `stage`：阶段进入运行中或已完成，并带当前阶段状态列表。
- `report`：该阶段刚产出的中间小结。
- `run`：最终完整工作流结果。
- `error`：可恢复错误信息。

这让页面能在评论读取、清洗、范围选择、分类、PRD、QA 和校验过程中持续更新，而不是等最终结果返回后再回放 `stageReports`。普通接口 `POST /workflow/runs` 仍保留，便于测试、脚本和非流式客户端使用。
...
- live 采集失败时，界面会提示改用导入评论。
- live 源返回 0 条评论时，界面会显示醒目的空数据提示，说明系统已停止生成无证据发现、需求和测试用例，并建议改用导入 JSON/CSV 或稍后重试。
...
本地开发时，后端会自动读取仓库根目录下的 `.env.local` 或 `.env`。不配置对应模型 key 时，系统会返回空分析结果，不会伪造 finding。配置 DeepSeek 后，语义 finding 会通过 DeepSeek 的 OpenAI 兼容 Chat Completions API 生成，并在结果中标记 `modelDriven: true`。

前端启动时会读取 `GET /config/model`，只展示 provider、模型、是否已配置 key 以及当前分析模式，不会返回 key 内容。缺少对应 key 时，工作台会明确提示当前不会生成模型发现。
...
- 没有模型 key 时，系统返回空分析结果，不生成发现、需求或测试用例。
- 模型服务不可用时，API 返回可恢复错误，提示检查 key 或网络。
- live 采集失败或 Apple RSS 返回空数据时，界面提示导入评论。
...
- 原始评论与清洗后的评论
- 分类结果，包括模型方法、范围评论、发现、样本数、置信度和冲突证据数量
- 分析范围、范围评论 ID、范围过滤说明、过滤规则、排除评论 ID、证据信号、约束和不确定性
